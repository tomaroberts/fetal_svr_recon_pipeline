...
# REMINDER FOR TOMO:
#
# - Previously called: test_autoReconstruct.sh
#
# - Old code, but contains code for making the landmark-based reconstructions easier
# - This is not implemented in the current fetal reconstruction pipeline.
# - Think I made this for general usage, but it's not widely used, so I've not folded it into the rest of the code.
# - In future, might want to pick out the landmark code and fold into main scripts.
...
### Reorder SVR strings if target stack is not stack1
#---------------------------------------------------------------------------------
...
targetDofFile=(../stack-$maskNum-$maskNum.dof)
tempDofFileNames=(${arrDofFiles[@]/$targetDofFile})
arrDofFiles=("$targetDofFile" ${tempDofFileNames[@]})
...
if [ $numVtkFiles -gt 0 ]; then	#untidy, loop could be removed - purely for -dofin input...
	...
	# with Landmarks
	CMD="reconstruction ../outputSVRvolume.nii.gz \
					$numStacks \
					${arrStackNames[*]} \
					-dofin ${arrDofFiles[*]} \
					-mask ../$maskName \
