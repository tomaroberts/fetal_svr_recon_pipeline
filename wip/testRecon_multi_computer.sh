...
# while true; do
	# for lockfile in /tmp/recon_loc1 /tmp/recon_loc2; do
		...
		# then
			...
			ssh ${host} "export PATH=/pnraw01/dhcp-reconstructions/FetalPreprocessing/bin/irtk_bin:$PATH; cd ${wdir}/${patientDir}/recon; /pnraw01/dhcp-reconstructions/FetalPreprocessing/bin/max_jobs.bash ../outputSVRvolume.nii.gz \
																	$numStacks \
																	${arrStackNames[*]} \
																	-mask ../$maskName \
																	-packages ${arrPackages[*]} \
